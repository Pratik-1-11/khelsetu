...
export class BadmintonScoringEngine extends BaseScoringEngine {
  constructor(sportConfig) {
    super(sportConfig);
    this.sets = this.rules.sets || 3;
    this.pointsPerSet = this.rules.points_per_set || 21;
    this.winByTwo = this.rules.win_by_2 || true;
  }

  validateEvent(eventType, eventData) {
    const validTypes = Object.values(BADMINTON_EVENT_TYPES);
  ...
  calculateScore(events) {
    return events.filter(e => !e.is_reversed && e.event_type === BADMINTON_EVENT_TYPES.POINT).length;
  }
  ...
  getMatchSummary(events) {
    ...
    for (let i = 1; i <= this.sets; i++) {
      const setEvents = events.filter(e =>
        !e.is_reversed &&
        e.metadata?.set === i &&
        e.event_type === BADMINTON_EVENT_TYPES.POINT
      );
      sets[`set${i}`] = {
...
export default BadmintonScoringEngine;